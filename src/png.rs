...
impl core::convert::TryFrom<&[u8]> for Png {
    type Error = Error;
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        while i < value.len() {
            ...
            i += chunk.length() as usize + Chunk::METADATA_BYTES;
            chunks.push(chunk);
        }
...
impl std::fmt::Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "Png {{",)?;
        writeln!(f, "  Header: [",)?;
        for e in 0..self.header.len() {
            if e == self.header.len() - 1 {
                writeln!(f, "       {}", e)?;
                continue;
            }
            writeln!(f, "       {},", e)?;
